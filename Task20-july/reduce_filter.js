...
*/
function avgAge(age) {
  let age1= age.reduce(function(acc,current) {
...
*/
function longestName(full) {
  let full1= full.reduce(function(acc,current) {
   return acc.first+acc.last > current.first+current.last ? acc: current;
  });
  console.log(full1.name.first,full1.name.last);
}
longestName(persons);
...
function maxNumber(num) {
  let num1= num.reduce(function(acc,current) {
...
function repeatChar(string,Char) {
  let count=0;
  let repeatChar1= string.split("").reduce(function(acc,current) {
    return Char==current ? count++ : false;
...
function between(n1, n2) {
  ...
  console.log(`usAndNumberBeetweenUs(${n1},${n2}) => [${emptyarray}]`);
}
between(2,5);
